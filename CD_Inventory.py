...
# ALarson, 2021-Dec-12, Added sub menu functionality and ability to manage CD tracks
#------------------------------------------#
    ...
    if strChoice == 'l':
        ...
        while True: 
            ...
            if strChoice == 'a':
                ...
                PC.DataProcessor.add_track(trkId, trkTitle, trkLength)
                IO.ScreenIO.show_tracks(cd)
            elif strChoice == 'd':
                IO.ScreenIO.show_tracks(cd)
            elif strChoice == 'r': 
                IO.ScreenIO.show_tracks(cd)
                trk_remv = input('Which track would you like to remove?')
                cd.rmv_track(trk_remv)   
            elif strChoice == 'x':
                break
            else: 
                raise Exception('Please select a sub-menu selection (a, d, r or x).')
    elif strChoice == 's':
        IO.ScreenIO.show_inventory(lstOfCDObjects)
