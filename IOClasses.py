...
# ALarson, 2021-Dec-12, Modified save_inventory and load_inventory to accept a list of files
#------------------------------------------#
    ...
    def save_inventory(file_name: list, lst_Inventory: list) -> None:
        ...
        try:
            with open(file_name_cd, 'w') as file:
                for eachCD in lst_Inventory:
                    file.write(eachCD.get_record())
            with open(file_name_track, 'w') as file:
                for eachCD in lst_Inventory:
                    tracks = eachCD.cd_tracks
                    cdid = eachCD.cd_id
                    for eachTrack in tracks:
                        strTrack = str(cdid) + ',' + eachTrack.get_record()
                        file.write(strTrack)
